...
def makeGen(cha, NUMLABELS):
    ...
    x = adaBlock(x3,inp_target,cha*16)
    x = adaBlock(x,inp_target,cha*16)

    x = Concatenate(axis=-1)([x,x3])    #An example of a skip connection: Earlier data is concatenated channel wise to the current data.
